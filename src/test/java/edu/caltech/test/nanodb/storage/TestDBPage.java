...
public class TestDBPage extends StorageTestCase {
    ...
    public void beforeClass() {
        ...
        dbFile = fileMgr.createDBFile("TestDBPage_TestFile", type, pageSize);
    ...
    public void testReadWriteVarString65535() {
        ...
        for (int i = 0; i < 32; i++) {
            buf.append("A SURGE in the radiation levels surrounding the " +
                "reactors at the Dai-ichi nuclear power plant at Fukushima " +
                "on Wednesday morning forced authorities to withdraw " +
                "workers from the site of Japan's escalating nuclear " +
                "catastrophe. A skeleton crew of 50, these are");
        }

        // add 28 characters to be 8188
        buf.append("A SURGE in the radiation lev");
        value = buf.toString();
        assert value.length() == 8188 :
            "test string length is not 8188 (it's " + value.length() + ")";
